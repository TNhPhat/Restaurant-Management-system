...
std::vector<std::shared_ptr<Employee>> EmployeeRepositoryInfo::GetAllEmployeeInfo(){
    ...
    LOG_INFO("Successfully loaded {} employees infomation from {}",std::to_string(EmployeeList.size()),this->m_FileHandler->GetFilePath());
    return EmployeeList;
}
...
void EmployeeRepositoryAttendance::SaveEmployeeAttendaceRecord(std::vector<std::shared_ptr<AttendanceRecord>>& record){
    json data = json::array();
    for(auto it: record)
