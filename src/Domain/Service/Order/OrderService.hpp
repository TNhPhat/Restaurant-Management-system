...
public:
    ...
    std::vector<std::shared_ptr<Order>> SortByDate(const std::vector<std::shared_ptr<Order>>& orders, bool ascending = true);

    std::vector<std::shared_ptr<Order>> SortByTotal(const std::vector<std::shared_ptr<Order>>& orders, bool ascending = true);
};