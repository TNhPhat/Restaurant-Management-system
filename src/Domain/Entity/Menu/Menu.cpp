...
void MenuItem::RemoveAddon(const std::shared_ptr<MenuAddon> &Addon) {
    ...
    for (const auto &existingAddon: this->m_AvailableAddons) {
        if (existingAddon->GetID() == Addon->GetID()) {
            ...
            LOG_INFO("Removed MenuAddon with ID {} from MenuItem with ID {}.", Addon->GetID(), this->GetID());
            return;
        }
    }
    // LOG_WARNING("MenuAddon with ID {} not found in MenuItem with ID {}.", Addon->GetID(), this->GetID());
}
...
void MenuSection::RemoveItem(const int ItemID) {
    ...
    LOG_WARNING("MenuItem with ID {} not found in MenuSection with ID {}.", ItemID, this->GetID());
}

std::shared_ptr<MenuItem> MenuSection::GetItemByID(int ItemID) {
    for (const auto &Item: this->m_MenuItems) {
        if (Item->GetID() == ItemID) {
            return Item;
        }
...
std::shared_ptr<MenuItem> MenuSection::GetItemByTitle(const std::string &Title) {
    for (const auto &Item: this->m_MenuItems) {
...
void Menu::RemoveSection(const int SectionID) {
    for (const auto &Section: this->m_Sections) {
        if (Section->GetID() == SectionID) {
            std::erase(this->m_Sections, Section);
            LOG_INFO("Removed MenuSection with ID {} from Menu with ID {}.", SectionID, this->GetID());
            return;
        }
    }
    LOG_WARNING("MenuSection with ID {} not found in Menu with ID {}.", SectionID, this->GetID());
}
